...
class ProjectContribution(db.Model):
    ...
    user_id = db.Column(db.String(100), db.ForeignKey('users.user_id'), nullable=False, comment='贡献者Auth0用户标识')
    ...
    contributor = db.relationship('User', backref='contributions', foreign_keys=[user_id])
    ...
    def to_dict(self):
        ...
        if self.project:
            project_info = {
                ...
                'end_time': self.project.end_time.strftime('%Y-%m-%d %H:%M:%S') if self.project.end_time else None,
                'created_at': self.project.created_at.strftime('%Y-%m-%d %H:%M:%S') if self.project.created_at else None
            }
