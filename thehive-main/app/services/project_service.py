...
class ProjectService:
    ...
    def get_project_list(filters=None):
        ...
        for project in projects:
            ...
            # 添加点赞数
            project_dict['likes_count'] = ProjectLike.query.filter_by(project_id=project.id).count()
    ...
    def add_or_remove_project_like(project_id, user_id):
        ...
        """
        existing_like = ProjectLike.query.filter_by(project_id=project_id, user_id=user_id).first()
        if existing_like:
    ...
    def get_top_liked_projects(limit=3):
        ...
        top_projects = db.session.query(
            Project,
            func.count(ProjectLike.id).label('likes_count')
        ).outerjoin(ProjectLike).group_by(Project.id).order_by(func.count(ProjectLike.id).desc()).limit(limit).all()
...
class ProjectDeliverableService:
    ...
    def create_deliverable(project_id, data, uploader_id, file=None):
        ...
        deliverable = ProjectDeliverable(
            ...
            file_name=data.get('file_name') or filename, # 如果没有提供file_name，使用上传的文件名
            file_size=file_size,
    ...
    def get_deliverables_by_project(project_id):
        ...
        :return: 交付物列表
        """
        deliverables = ProjectDeliverable.query.filter_by(project_id=project_id).order_by(
            ProjectDeliverable.created_at.desc()
        ).all()
...
class DeliverableConfirmationService:
    ...
    def get_project_confirm_status(project_id, user_id):
        ...
        if not approved_applications: # 如果用户不是该项目的参与者，则无法确认交付物
            return {'total_deliverables': 0, 'confirmed_deliverables': 0, 'all_confirmed': False}
        
        # 获取项目的所有交付物
        all_deliverables = ProjectDeliverable.query.filter_by(project_id=project_id).all()
        ...
        return {
            ...
            'all_confirmed': total_deliverables > 0 and total_deliverables == confirmed_deliverables_count
        }
    ...
    def check_and_complete_project_by_confirmation(project_id):
        ...
        for participant_app in approved_participants:
            ...
            status = DeliverableConfirmationService.get_project_confirm_status(project_id, participant_user_id)
