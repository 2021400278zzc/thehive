...
def create_project():
    ...
    try:
        project = app.services.project_service.ProjectService.create_project(data, user_id)
...
def get_founder_projects():
    ...
    try:
        projects = app.services.project_service.ProjectService.get_founder_project_list(filters)
        return jsonify({'data': projects, 'total': len(projects)}), 200
    except Exception as e:
        return jsonify({'error': f'获取项目列表失败: {str(e)}'}), 500
...
def get_participant_projects():
    ...
    try:
        projects = app.services.project_service.ProjectService.get_participant_project_list(filters)
        return jsonify({'data': projects, 'total': len(projects)}), 200
    except Exception as e:
        return jsonify({'error': f'获取项目列表失败: {str(e)}'}), 500
...
def get_project_detail(project_id):
    ...
    """
    try:
        project = app.services.project_service.ProjectService.get_project_detail(project_id)
...
def like_project(project_id):
    ...
    print(f"Has add_or_remove_project_like: {'add_or_remove_project_like' in dir(app.services.project_service.ProjectService)}")
    
    try:
        liked = app.services.project_service.ProjectService.add_or_remove_project_like(project_id, user_id)
        if liked:
...
def get_project_leaderboard():
    ...
    """
    try:
        top_projects = app.services.project_service.ProjectService.get_top_liked_projects()
...
def process_application(application_id):
    ...
    try:
        application = ProjectApplicationService.process_application(application_id, data['status'], data.get('response_message'), user_id)
...
def update_project(project_id):
    ...
    try:
        project = app.services.project_service.ProjectService.update_project(project_id, data, user_id)
...
def delete_project():
    ...
    try:
        result = app.services.project_service.ProjectService.delete_project(project_id, user_id)
        return jsonify({'message': result['message']}), 200
    except ValueError as e:
        return jsonify({'error': str(e)}), 400
    except Exception as e:
...
def upload_project_deliverable():
    ...
    if not all([project_id, uploader_id]) or (not file and not file_url and not link_url):
...
def delete_project_deliverable():
    ...
    try:
        result = ProjectDeliverableService.delete_deliverable(deliverable_id, uploader_id)
        return jsonify({'message': result['message']}), 200
    except ValueError as e:
        return jsonify({'error': str(e)}), 400
    except Exception as e:
...
def update_deliverable_status():
    ...
    try:
        deliverable = ProjectDeliverableService.update_status(deliverable_id, status, reviewer_id)
...
def complete_project_by_confirmation():
    ...
    try:
        DeliverableConfirmationService.check_and_complete_project_by_confirmation(project_id)
